...
def init_db() -> None:
    ...
    with sqlite3.connect(DB_PATH) as conn:
        ...
        add_column_if_missing(conn, "events", "ts", "TEXT")
        add_column_if_missing(conn, "events", "host_type", "TEXT")
        add_column_if_missing(conn, "events", "service", "TEXT")
        add_column_if_missing(conn, "events", "app_stack", "TEXT")
        add_column_if_missing(conn, "events", "event_class", "TEXT")
        add_column_if_missing(conn, "events", "dependency", "TEXT")
        add_column_if_missing(conn, "events", "canonical_fingerprint", "TEXT")
        add_column_if_missing(conn, "events", "incident_id", "INTEGER")
        add_column_if_missing(conn, "events", "noise_score", "REAL DEFAULT 0")
        add_column_if_missing(conn, "events", "severity_norm", "TEXT")
        add_column_if_missing(conn, "events", "message_template", "TEXT")
        add_column_if_missing(conn, "events", "labels", "TEXT DEFAULT '{}'")
        ...
        add_column_if_missing(conn, "incidents", "analysis_json", "TEXT")
        add_column_if_missing(conn, "incidents", "probable_root_cause", "TEXT")
        add_column_if_missing(conn, "incidents", "confidence", "TEXT")
        add_column_if_missing(conn, "incidents", "last_analyzed_at", "TEXT")
        conn.execute(
...
def infer_host_type(host: str) -> str:
    meta = NODE_METADATA.get(host or "", {})
...
def infer_labels(host: str, container: str) -> dict[str, Any]:
    labels: dict[str, Any] = {}
    node_meta = NODE_METADATA.get(host or "", {})
...
def _is_low_value_nearby_event(row: sqlite3.Row) -> bool:
    severity_norm = (row["severity_norm"] or "").strip().lower()
    event_class = (row["event_class"] or "").strip().lower()
    container = (row["container"] or "").strip().lower()
    message = (row["message"] or "").strip().lower()
    ...
    # Optional: suppress routine syslog info noise from generic syslog container
...
def _summarize_events(rows: list[sqlite3.Row]) -> dict[str, Any]:
...
def build_incident_context_filtered(
    ...
    for item in base["nearby_events"]:
        ...
        if exclude_noise:
            fake_row = {
                "severity_norm": item.get("severity_norm", ""),
                "event_class": item.get("event_class", ""),
                "container": item.get("container", ""),
                "message": item.get("message", ""),
            }
            if _is_low_value_nearby_event(fake_row):  # type: ignore[arg-type]
                continue

        filtered_nearby.append(item)

    base["nearby_events_filtered"] = filtered_nearby
    base["nearby_stats_raw"] = {
        "total": 0,
        "by_host": {},
        "by_severity": {},
        "by_event_class": {},
    ...
    # Recompute stats from dict rows
...
def generate_open_incidents_digest(limit: int = 10, include_raw_response: bool = False) -> dict[str, Any]:
    incidents = fetch_open_incidents_for_digest(limit=limit)
